        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-7">
          {filteredData.map((item) => (
            <div className="w-full border p-3 rounded-lg shad" key={item._id}>
              ...
              <h2 className="text-lg font-semibold text-gray-700">
                Product Quantity: <span className="text-black">{item.productQuantity}</span>
              </h2>
              <h2 className="text-lg font-semibold text-gray-700">
                Availability: <span className="text-black">{item.availibility}</span>
              </h2>
              <h2 className="text-lg font-semibold text-gray-700">
                Added Date: <span className="text-black">{item.addedDate}</span>
              </h2>
              <div className="flex justify-center gap-5 mt-5 mb-2">
                <Link to={`/updateasset/${item._id}`} className="w-full bg-cyan-500 text-center text-md text-white py-1 rounded-md font-semibold">
                  Update
                </Link>
                <button onClick={() => handleDelete(item._id)} className="w-full bg-red-500 text-md text-white py-1 rounded-md font-semibold">
                  Delete
                </button>
              </div>
            </div>
