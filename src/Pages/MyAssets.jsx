      ...
      <div className="w-full min-h-screen bg1">
        <div className="contain pt-1 pb-10 px-2 md:px-3 lg:px-0">
          <div className="mb-4 grid  grid-cols-1 md:grid-cols-3 w-full gap-3 border-2 p-1 rounded-lg border-purple">
            ...
            <select onChange={handleStatusChange} className="p-2 border rounded-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-7">
            {filteredData.map((item) => (
              <div className="w-full border p-3 rounded-lg shad" key={item._id}>
                ...
                <h2 className="text-lg font-semibold text-gray-700">
                  Requested Date: <span className="text-black">{item.requestedDate}</span>
                </h2>
                <h2 className="text-lg font-semibold text-gray-700">
                  Approval Date: <span className="text-black">{item.approvalDate || "Not Approved Yet"}</span>
                </h2>
                <h2 className="text-lg font-semibold text-gray-700">
                  Request Status: <span className="text-black">{item.status}</span>
                </h2>
                <div className="flex justify-center gap-5 mt-5 mb-2">
                  ...
                  {
                    item.status === "Approved" && item.productType === "Returnable" &&
                    <button onClick={() => handleReturn(item._id, item.productId)} className="w-full bg-purple text-md text-white py-1 rounded-md font-semibold">
                      Return
                    </button>
                  }
                  {
                    ...
                    >
                      {({ loading }) => (loading ? 'Loading document...' : 'Print as PDF')}
                    </PDFDownloadLink>
                  }                
                </div>
              </div>
