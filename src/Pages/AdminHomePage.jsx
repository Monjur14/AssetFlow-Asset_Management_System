...
const AdminHomePage = () => {
  ...
  const calculateTopItems = (filteredData) => {
    ...
    const topRequestedItems = productCountArray.slice(0, 4).map(([name, info]) => ({
      name,
      image: info.image
    }));
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4 lg:gap-7 mt-5">
            {limitedStockItems.map((item) => (
              <div className="w-full border p-3 rounded-lg shad" key={item._id}>
                ...
                <h2 className="text-base font-semibold text-gray-700">
                  Quantity: <span className="text-black">{item.productQuantity}</span>
                </h2>
              </div>
            ))}
          </div>
        ...
        <div className="mt-10 lg:mt-20 pb-20">
          ...
          <div className="flex justify-center mt-5">
            <div className="max-w-[500px] p-5 border rounded-lg shad">
              ...
              <h2 className="text-base font-semibold text-gray-700">
                Returnable Requests: <span className="text-black">{statisticsSummary.returnableRequests}</span>
              </h2>
              <h2 className="text-base font-semibold text-gray-700">
                Non-Returnable Requests: <span className="text-black">{statisticsSummary.nonReturnableRequests}</span>
              </h2>
              <h2 className="text-base font-semibold text-gray-700">
                Limited Stock Items: <span className="text-black">{statisticsSummary.limitedStockItems}</span>
              </h2>
            </div>
          </div>
        </div>
